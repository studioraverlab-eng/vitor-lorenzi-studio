...
export default function FloatingWhatsAppButton() {
  ...
  useEffect(() => {
    const targets = document.querySelectorAll('#contato, #portfolio-contact')
    if (!targets.length) return

    const observer = new IntersectionObserver(
      entries => setNearContact(entries.some(entry => entry.isIntersecting)),
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          >
            <span className="absolute inset-0 hidden rounded-full animate-ping opacity-25 sm:block" style={{ background: '#25D366' }} />
